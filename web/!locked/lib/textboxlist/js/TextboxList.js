...
$.TextboxList = function(element, _options){
    ...
    inBetweenEditableBits: true,
		...
		placeholder: 'Appuyez sur ENTRÉE après chaque mot clé.',
    plugins: {},
	...
	var init = function(){		
		...
		container = $('<div class="relative '+options.prefix+'" />')
			.insertAfter(element)
			.click(function(e){ 
		...
		if (chk(options.placeholder) && original[0].value == '') placeholder = $('<div style="pointer-events:none;padding: 20px 15px;position:absolute;color:rgba(23,44,74,.5);font-size: 1.8rem;font-family: Outfit, sans-serif, sans-serif;font-weight: 400;" class="'+ options.prefix +'-placeholder" />').html(options.placeholder).appendTo(container);		
