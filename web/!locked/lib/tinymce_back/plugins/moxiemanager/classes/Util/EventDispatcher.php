...
class MOXMAN_Util_EventDispatcher {
	...
	/*public function dispatch($sender, $name, $args) {	
		...
		if (isset($this->observers[$name])) {
			...
			for ($i = 0, $l = count($observers); $i < $l; $i++) {
				$func = $observers[$i][0];
				$value = null;
				if($value != null && function_exists($func)){
					$_value = $func($args);
					$value = $observers[$i][1]->$_value;
					print_r($value);
				}
	...
	public function dispatch($sender, $name, $args) {
		...
		if (isset($this->observers[$name])) {
			...
			$args->setSender($sender);
			for ($i = 0, $l = count($observers); $i < $l; $i++) {
				if($observers[$i][0] == 'onFileAction' && $i == 1) 
					$value = $observers[$i][1]->onFileAction($args);
				else{
					$func = $observers[$i][0];
						$value = null;
						if($value != null && function_exists($func)){
							$_value = call_user_func($func,$args);
							$value = $observers[$i][1]->$_value;
						}
				}
