...
var chart_stackedBar = new CanvasJS.Chart("chartContainer_<?php echo $key;?>", {
	...
	data: [
		...
		//foreach($renderstats->output as $output => $group){?>{
				...
				name: "USER",
				axisYType: "secondary",
				<?php /*if($i_incr == ($starter->utils->is__countable($renderstats->output) ? count($renderstats->output) : 0 )){?>
					indexLabel: "#total",
					indexLabelPlacement: "outside",
				<?php }*/?>
				dataPoints: [
					<?php if(isset($renderstats->output['stat'])) foreach($renderstats->output['stat'] as $stat => $page){?>{
